    ...
    router.post("/",(req,res) => {
      message=[];
        ...
        contractId = req.body.ContractId;
            sqlRequest.query(parse("SELECT  cn.Vegetable AS ExporterVegetables,  cn.Weight AS Weight, fm.Username AS name, cn.Comment AS comment, cn.EndDate AS EndDate  FROM dbo.Farmer AS fm JOIN dbo.Contract AS cn ON fm.id = cn.FarmId WHERE cn.Contractid = '%s' AND cn.Accepted = 'Yes' AND cn.ExporterRead='No'",contractId),(err,rows) => {
                if(err){console.log(err);}
                else{
